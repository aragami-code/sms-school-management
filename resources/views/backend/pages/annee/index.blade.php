...
@section('title')
liste des D'Années| Tableau de Bord
@endsection
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">liste des d'année </h4>
                    @include('backend.layouts.partials.messages')
                    @if(Auth::guard('admin')->user()->can('annee.create'))
                    <p class="float-right">
                    <a href="javascript:void(0)" class="btn btn-success mb-2" id="create-new-post"> <b><i class="fa fa-plus"></i></b><span>
                    </span>Creer une nouvelle année</a>
                    ...
                    @if(Auth::guard('admin')->user()->can('annee.view'))
                    <div class="data-tables">
                        <table id="dataTable" class="text-center">
                            <thead class="bg-light text-capitalize">
                                <tr>
                                   <th width="20%">Nom de l'année</th>
                                    <th width="30%">Nom</th>
                            ...
                            <tbody id="posts-crud">
                              ...
                              <tr id="id_{{ $annee->id }}">
                                    ...
                                    <td>{{$annee->slug_annee}}</td>

                                    <td>

                                        @if(Auth::guard('admin')->user()->can('annee.edit'))

                                        <a href="javascript:void(0)" id="edit-post" data-id="{{ $annee->id }}" data_token="{{csrf_token()}}" class="btn btn-info">  <b><i class="fa fa-edit"></i></b><span>
                                        </span>Modifier</a>
    ...
    $(document).ready(function () {
      ...
      $('body').on('click', '#create-new-post', function () {
        $('#btn-save').val("create-post");
          ...
          $('#postCrudModal').html("Ajouter une Année");
          $('#ajax-crud-modal').modal('show');

     });
     $('body').on('click', '#edit-post', function () {
        var annee_id = $(this).data('id');
        $.get('annee/'+annee_id+'/edit', function (data) {
           $('#postCrudModal').html("Editer une Année");
            $('#btn-save').val("edit-post");
            $('#ajax-crud-modal').modal('show');
            $('#id').val(data.id);
            $('#name_annee').val(data.name_annee);
            $('#slug_annee').val(data.slug_annee);
        })
     });
                                    ...
                                    if (e.value === true)
                                     {
                                        ...
                                        $.ajax({
                                        type: 'POST',
                                        url: "{{url('/admin/annee/delete')}}/" + annee_id,
   ...
   if ($("#postForm").length > 0) {
       ...
       submitHandler: function(form) {
        ...
        swal({
                ...
                var annee = '<tr id="id_' +data.id+ '"><td>' +data.name_annee+ '</td>'+'</td><td>' +data.slug_annee+ '</td>';
